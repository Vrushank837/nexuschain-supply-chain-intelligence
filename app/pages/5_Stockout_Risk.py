...
header("Stockout risk", "Estimate the probability that a part will reach or fall below safety stock within 14 days")
try:
    inv = read_query("SELECT i.*, p.category, p.safety_stock, p.lead_time_days FROM inventory i JOIN parts p ON p.part_id=i.part_id WHERE i.date=(SELECT MAX(date) FROM inventory)")
    hist = read_query("SELECT part_id, AVG(consumed_quantity) AS demand_avg_7d, STDDEV(consumed_quantity) AS demand_std_7d, AVG(received_quantity) AS receipt_avg_14d FROM inventory GROUP BY part_id")
    if inv.empty:
        ...
        idx = st.selectbox("Part", df.index, format_func=lambda i: f"{df.loc[i,'part_id']} · {df.loc[i,'category']}")
        row = df.loc[[idx]]
        features = row[["part_id","category","closing_stock","safety_stock","lead_time_days","demand_avg_7d","demand_std_7d","receipt_avg_14d","days_of_cover","safety_stock_gap","month","day_of_week"]]
        ...
        st.dataframe(row[["part_id","closing_stock","safety_stock","lead_time_days","demand_avg_7d","days_of_cover"]], use_container_width=True, hide_index=True)
except Exception as exc:
    db_error(exc)
