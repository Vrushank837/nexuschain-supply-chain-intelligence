...
try:
    po = read_query("SELECT po_id, order_date, supplier_id, part_id, quantity, unit_price FROM purchase_orders WHERE status='open' ORDER BY order_date DESC LIMIT 200")
    ...
    if df.empty:
        ...
        idx = st.selectbox("Open order", df.index, format_func=lambda i: f"{df.loc[i,'po_id']} · {df.loc[i,'supplier_name']} · {df.loc[i,'part_id']}")
        row = df.loc[[idx]].copy()
        ...
        features = row[["supplier_id","part_id","category","lead_time_days","quantity","unit_price","month","day_of_week","supplier_prior_delay_rate","supplier_prior_avg_delay","supplier_prior_orders"]]
