...
header("Supplier intelligence", "Compare supplier reliability, capacity signals and delivery consistency")
try:
    ...
    with left:
        ...
        if not df.empty:
            selected = st.multiselect("Suppliers", df.supplier_id.tolist(), default=df.head(5).supplier_id.tolist())
            if selected:
                monthly = monthly[monthly.supplier_id.isin(selected)]
        fig = px.line(monthly, x="month", y="on_time_rate_pct", color="supplier_id", markers=True, title="On-time delivery trend")
        fig.update_layout(yaxis_title="On-time %", xaxis_title=None)
        st.plotly_chart(fig, use_container_width=True)
    with right:
        st.subheader("Supplier register")
        st.dataframe(df[["supplier_name","country","total_orders","on_time_rate_pct","avg_delay_days"]].head(12), use_container_width=True, hide_index=True)
