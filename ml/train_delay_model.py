...
NUM = ["lead_time_days", "quantity", "unit_price", "month", "day_of_week", "supplier_prior_delay_rate", "supplier_prior_avg_delay", "supplier_prior_orders"]
...
def train(output_dir: Path = settings.model_dir) -> dict:
    ...
    save_metrics({"task": "supplier_delay", "cutoff": str(cutoff.date()), "baseline": base_metrics, "xgboost": metrics}, output_dir / "supplier_delay_metrics.json")
    (output_dir / "supplier_delay_metadata.json").write_text(json.dumps({"categorical_features": CAT, "numeric_features": NUM, "target": "delay_flag", "cutoff": str(cutoff.date())}, indent=2), encoding="utf-8")
